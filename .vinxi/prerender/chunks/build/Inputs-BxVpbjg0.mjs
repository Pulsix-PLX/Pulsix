...
import { createMemo, createSignal, onMount, createEffect, Switch, Match, Show } from 'file://C:/Users/Matteo/Desktop/Pulsix/node_modules/.pnpm/solid-js@1.9.5/node_modules/solid-js/dist/server.js';
...
import { Y as Y$1, Q as Q$1 } from '../_/nitro.mjs';

const [I, r] = createStore({}), [M, j] = createStore({}), fe = createMemo(() => {
...
const Y = w(async (e) => {
  try {
    console.log(e);
    const a = await D.query("SELECT username FROM users WHERE username=$1", [e]);
...
}, "src_routes_API_Auth_registration_credentials_usernameAlreadyexist_ts--usernameAlreadyexist_action", "C:/Users/Matteo/Desktop/Pulsix/src/routes/API/Auth/registration/credentials/usernameAlreadyexist.ts?tsr-directive-use-server="), G = Q$1(Y, "usernameAlreadyexist"), J = w(async (e) => {
  try {
    console.log(e);
    const a = await D.query("SELECT phone_number FROM users WHERE phone_number=$1", [e]);
    return console.log(a.rows), a.rows.length > 0 ? "already exist" : "available";
  } catch (a) {
    return console.error("Error checking phone:", a), `error:${a.message}`;
  }
}, "src_routes_API_Auth_registration_phone_phoneAlreadyexist_ts--phoneAlreadyexist_action", "C:/Users/Matteo/Desktop/Pulsix/src/routes/API/Auth/registration/phone/phoneAlreadyexist.ts?tsr-directive-use-server="), Q = Q$1(J, "phoneAlreadyexist"), X = w(async (e) => {
  try {
    console.log(e);
    const a = await D.query("SELECT email FROM users WHERE email=$1", [e]);
...
export { fe as f, j, r, ve as v, ye as y };
//# sourceMappingURL=Inputs-BxVpbjg0.mjs.map
