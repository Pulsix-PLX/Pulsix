...
import { transactionFieldstype } from '~/server/types/transaction';
import { context, setStep } from '../context';
import ButtonSparkle from '~/components/Buttons/AnimatedIconButton/ButtonSparkle';
import { processCsvData } from './controls';

// Definizione della logica per determinare il tipo di transazione
export interface TransactionTypeLogic {
...
export default function Mapper() {
  ...
  const systemFields = [
    ...
    { id: 'subCategory_id', label: 'Sub Category' },
  ];
  ...
  const previewSystemIds = ['cause', 'amount', 'date'];

  //- Unica funzione di generazione Preview -//
  const transactionExamples = createMemo(() => {
    ...
    if (!context.data?.rows || context.data?.length === 0) {
    ...
    const sampleRows = context.data?.rows.slice(0, 5);
    ...
    const examples = sampleRows.map((row, index) => {
      ...
      if (typeLogic.mode === 'auto') {
        tipo = importo < 0 === typeLogic.negativeIsExpense ? 'Expense' : 'Income';
      } else if (
        ...
        if (tipoValore === typeLogic.positiveValue) tipo = 'Income';
        else if (tipoValore === typeLogic.negativeValue) tipo = 'Expense';
      }
  ...
  const submitMapping = async () => {
    console.log('Processing data before sending...');
    setError(null); // Resetta errori

    // Recupera dati grezzi e configurazione
    const rawData = context.data?.rows; // Assumendo context contenga i dati
    const mappingConfig = mappings(); // Lo stato delle mappature
    const typeLogicConfig = transactionTypeLogic(); // Lo stato della logica tipo

    // Verifica se i dati grezzi esistono
    if (!rawData || rawData.length === 0) {
      setError("Nessun dato da processare.");
      return;
    }

    // *** Chiama la nuova funzione di processamento ***
    const processedData = processCsvData(rawData, mappingConfig, typeLogicConfig);

    console.log('Dati processati pronti per l\'invio:', processedData);

    // Verifica se ci sono dati validi da inviare
    // (potresti voler filtrare le righe con amount: null se non le vuoi inviare)
    const validDataToSend = processedData.filter(tx => tx.amount !== null && tx.type !== null);

    if (validDataToSend.length === 0) {
        setError("Nessuna transazione valida trovata dopo il processamento. Controlla la mappatura e i dati.");
        return;
    }
    ...
    try {
        ...
        // Qui potresti voler passare i dati processati o un messaggio di successo
        // props.onMappingComplete(/* ... */);
        setStep('complete'); // Esempio per andare allo step successivo

    } catch (error: any) {
      console.error("Errore durante l'invio dei dati processati:", error);
      setError(error.response?.data?.message || error.message || "Errore durante l'invio dei dati al server");
    }
  };
                        ...
                        <tr class="border-b border-gray-200">
                          ...
                          <td
                            class={`p-2 font-medium ${
                              example.interpretation === 'Income'
                                ? 'text-green-600'
                                : example.interpretation === 'Expense'
                                ? 'text-red-600'
                                : 'text-gray-600'
                            }`}
                          >
