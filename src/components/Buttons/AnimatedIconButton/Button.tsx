...
export default function SparkleButton(props: SparkleButtonProps) {
  ...
  // Default star icon if none provided
/*  const defaultIcon = (
  ...
  // Default particle icon if none provided
...
"Use client"
  ...
  return (
    <div class="sparkle-button">
      ...
      {isMounted() && (
        <span aria-hidden="true" class="particle-pen" style={{"--active": isActive() ? "1" : "0"}}>
          <For each={particles}>
            {(particle, index) => (
              <span 
                class="particle" 
                // Add a key for stability
                data-key={index()}
                style={{
                  "--x": `${particle.x}`,
                  "--y": `${particle.y}`,
              ...
              >
                {props.particleIcon/* || defaultParticleIcon*/}
              </span>
            )}
          </For>
        </span>
