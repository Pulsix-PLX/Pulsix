...
import { createSignal, Show, createResource, Switch, Match, Suspense, lazy } from 'file://C:/Users/Matteo/Desktop/Pulsix/node_modules/.pnpm/solid-js@1.9.5/node_modules/solid-js/dist/server.js';
import { S, x, A as A$1 } from './prova-UkNyxD49.mjs';
import { m, L } from './index-CM2EfUOf.mjs';
import { A } from './components-CJF4pMQg.mjs';
import { w as we } from './Inputs-CEYxPBfP.mjs';
import { p, D, x as x$1, f } from './deleteWallet-D6_HIjzQ.mjs';
import { B as B$1 } from './ButtonSparkle-DNpTyev3.mjs';

const R = E(async function(r$1) {
  var _a;
...
}, "src_routes_API_Wallets_getWallets_server_ts--getWallets_1", "C:/Users/Matteo/Desktop/Pulsix/src/routes/API/Wallets/getWallets.server.ts?tsr-directive-use-server="), re = R, M = E(async function(r$1, l) {
  var _a;
  const n = "[Server Function:getWalletsContainerEnhancedWithDeleteCheck]";
  if (console.log(`${n} Fetching per userId: ${r$1}, containerId: ${l}`), isNaN(r$1)) throw console.error(`${n} User ID non valido fornito.`), new Error("User ID non valido fornito.");
  ...
  try {
    const a = `
        ...
        `, s = i ? [r$1] : [r$1, l];
    return console.log(`${n} Query: ${a.replace(/\s+/g, " ").trim()}, Params: ${JSON.stringify(s)}`), (_a = (await r.query(a, s)).rows) != null ? _a : [];
  } catch (a) {
...
}, "src_routes_API_Wallets_getWallets_server_ts--getWalletsContainer_1", "C:/Users/Matteo/Desktop/Pulsix/src/routes/API/Wallets/getWallets.server.ts?tsr-directive-use-server="), le = M;
E(async function(r$1) {
  var _a, _b, _c;
  const l = "[Server Function:getRecursiveWalletBalance]";
  if (console.log(`${l} Calcolo somma per container ID: ${r$1}`), typeof r$1 != "number" || isNaN(r$1) || r$1 <= 0) throw console.error(`${l} ID container non valido fornito: ${r$1}`), new Error("ID container di partenza non valido.");
  try {
    const n = `
    ...
    const s = (_c = (_b = (_a = (await r.query(n, i)).rows) == null ? void 0 : _a[0]) == null ? void 0 : _b.total_balance) != null ? _c : 0;
    return console.log(`${l} Somma calcolata: ${s} per container ID: ${r$1}`), s;
  } catch (n) {
    throw console.error(`${l} Errore DB durante calcolo somma ricorsiva per ${r$1}:`, n), n.code && console.error(`DB Error Code: ${n.code}, Message: ${n.message}`), new Error(`Errore recupero somma wallets per container ${r$1}.`);
  }
...
}, "src_routes_API_Wallets_getWallets_server_ts--getWalletName_1", "C:/Users/Matteo/Desktop/Pulsix/src/routes/API/Wallets/getWallets.server.ts?tsr-directive-use-server="), oe = U;
...
const V = lazy(() => import('./index-WwoiZKEg.mjs').then((e) => ({ default: e.DefaultColorPicker })));
function ae() {
  const [e] = createResource(m(), p), [r, l] = createSignal(), [n, i] = createSignal(false), [a, s] = createSignal(null);
  return createComponent(Show, { get when() {
    return e();
  }, get children() {
    return [createComponent(Switch, { get children() {
      return [createComponent(Match, { get when() {
        ...
        var _a, _b;
        return [ssr(I, ssrHydrationKey()), ssr(P, ssrHydrationKey() + ssrAttribute("action", escape(D, true), false), "border:3px solid rgba(255, 255, 255, 0.3);border-radius:40px", ssrAttribute("value", escape((_a = e()) == null ? void 0 : _a.id, true), false), ssrAttribute("value", escape((_b = e()) == null ? void 0 : _b.type, true), false), escape(createComponent(we, { type: "text", name: "walletName", placeholder: "Wallet Name", get defaultValue() {
          var _a2;
        ...
        }, class: "ml-[auto] mr-[auto]", onClick: () => setTimeout(() => L(null), 500) })), ` text-white font-bold py-2 px-4 rounded ml-[auto] mr-[auto] mt-[1vw] ${escape(x$1.buttonDelete, true)}`, "border:2px solid red;border-radius:20px;width:100px;height:50px")];
      } }), createComponent(Match, { get when() {
        var _a;
        return ((_a = e()) == null ? void 0 : _a.type) == "wallet";
      }, get children() {
        var _a, _b, _c;
        return [ssr(T, ssrHydrationKey()), ssr(z, ssrHydrationKey() + ssrAttribute("action", escape(D, true), false), `border:3px solid ${escape(r(), true)};border-radius:40px`, ssrAttribute("value", escape((_a = e()) == null ? void 0 : _a.id, true), false), ssrAttribute("value", escape((_b = e()) == null ? void 0 : _b.type, true), false), escape(createComponent(we, { type: "text", name: "walletName", placeholder: "Wallet Name", get defaultValue() {
          var _a2;
          return (_a2 = e()) == null ? void 0 : _a2.wallet_name;
          ...
          return `${r()}`;
        }, shadow: 10, onClick: () => setTimeout(() => L(null), 500) })), ` text-white font-bold py-2 px-4 rounded ml-[auto] mr-[auto] mt-[1vw] ${escape(x$1.buttonDelete, true)}`, "border:2px solid red;border-radius:20px;width:100px;height:50px")];
      } })];
    } }), createComponent(Show, { get when() {
      return n();
    }, get children() {
      return [ssr(B, ssrHydrationKey()), ssr(H, ssrHydrationKey(), `background-color:#151515;border-radius:40px;border:3px solid ${escape(r(), true) || "#ffffffff"}`, `${escape(x$1.text, true)} text-[1.4vw] font-bold text-center`, `${a() == "cancel" ? escape(x$1.textCancel, true) : a() == "confirm" ? escape(x$1.textDelete, true) : escape(x$1.paragraf, true)} text-[1vw] font-bold text-center mt-[2vw]`, `${a() == "cancel" ? escape(x$1.textCancel, true) : a() == "confirm" ? escape(x$1.textDelete, true) : escape(x$1.paragraf, true)} text-[1vw] font-bold text-center mt-[0.5vw]`, escape(createComponent(B$1, { text: "Cancel", onClick: () => i(false), shadowColor: "hsl(0, 100%, 54%)", onMouseEnter: () => s("cancel"), onMouseLeave: () => s(null), shadow: 10 })), escape(createComponent(B$1, { shadowColor: "hsla(155, 100%, 50%, 0.922)", text: "Delete", shadow: 10, onClick: async () => {
        var _a, _b;
        await f((_a = e()) == null ? void 0 : _a.id, (_b = e()) == null ? void 0 : _b.type), setTimeout(() => L(null), 500);
      }, onMouseEnter: () => s("confirm"), onMouseLeave: () => s(null) })))];
    } })];
  } });
}

export { ae as a, le as l, ne as n, oe as o, re as r };
//# sourceMappingURL=index-B54VJo2T.mjs.map
