...
import { _ as _$1 } from './Card.module-nMwE8ysR.mjs';
...
const D = E(async function() {
  ...
  return null;
}, "src_Server_auth_server_ts--getUserId_1", "C:/Users/Matteo/Desktop/Pulsix/src/Server/auth.server.ts?tsr-directive-use-server="), q = D;
E(async function() {
  var _a, _b;
...
const O = "_backCardHolder_8wvpa_2", V = { backCardHolder: O }, L = E(async function(e) {
  ...
  try {
    const s = await r.query("SELECT rate FROM exchange_rates WHERE currency_code = $1 ORDER BY observation_date DESC LIMIT 1", [e.toUpperCase()]);
    if (s.rows.length > 0) {
...
}, "src_routes_API_exchangeRates_exchangeRates_ts--getExchangeRate_1", "C:/Users/Matteo/Desktop/Pulsix/src/routes/API/exchangeRates/exchangeRates.ts?tsr-directive-use-server="), C = L, A = E(async function(e, s) {
...
}, "src_routes_API_exchangeRates_exchangeRates_ts--getConversionRate_1", "C:/Users/Matteo/Desktop/Pulsix/src/routes/API/exchangeRates/exchangeRates.ts?tsr-directive-use-server="), U = A, M = E(async function(e, s, o) {
  var _a;
  ...
  const r$1 = s.toUpperCase();
  console.log(`${t} Inizio per container ${e != null ? e : "ROOT"} -> ${r$1}, user ${o}`);
  let a = 0;
  const i = [];
  try {
    const u = `
    ...
    const d = (_a = (await r.query(u, E)).rows) != null ? _a : [];
    if (console.log(`${t} Trovati ${d.length} wallet validi (non eliminati, con saldo e valuta) nella gerarchia.`), d.length === 0) return { total_balance: 0, currency_code: r$1 };
    const R = [...new Set(d.map((l) => l.currency.toUpperCase()).filter((l) => l !== r$1))], p = /* @__PURE__ */ new Map();
    if (R.length > 0) {
      console.log(`${t} Recupero tassi per: ${R.join(", ")} -> ${r$1}`);
      const l = R.map((c) => U(c, r$1).catch((g) => (console.error(`${t} Errore recupero tasso per ${c}:`, g), null))), h = await Promise.all(l);
      R.forEach((c, g) => {
        const f = h[g];
        if (p.set(c, f), f === null) {
          const m = `Tasso di cambio non trovato o errore per ${c} -> ${r$1}. I saldi in ${c} verranno ignorati.`;
    ...
    for (const l of d) {
      ...
      typeof g == "number" && !isNaN(g) ? a += Math.round(h * g * 100) / 100 : c !== r$1 && console.warn(`${t} Salto wallet ${l.id} (${h} ${c}) per tasso mancante/non valido.`);
    }
    return a = Math.round(a * 100) / 100, console.log(`${t} Calcolo completato. Totale in ${r$1}: ${a}`), { total_balance: a, currency_code: r$1, warnings: i.length > 0 ? i : void 0 };
  } catch (u) {
    throw console.error(`${t} Errore critico per container ${e != null ? e : "ROOT"} in ${r$1}, user ${o}:`, u), u instanceof Error && "code" in u && console.error(`DB Error Code: ${u.code}`), new Error(`Errore nel calcolo del totale convertito per container ${e != null ? e : "ROOT"}.`);
  }
}, "src_routes_API_exchangeRates_exchangeRates_ts--calculateConvertedTotal_1", "C:/Users/Matteo/Desktop/Pulsix/src/routes/API/exchangeRates/exchangeRates.ts?tsr-directive-use-server="), j = M;
E(async function(e, s, o) {
...
export { J, V, j, q };
//# sourceMappingURL=exchangeRates-ChMJm5Xh.mjs.map
