  ...
  try {
    ...
    if(data.get('type')=='wallet'){
     ...
     result = await db.query(
      ...
      [data.get('walletName'), data.get('type'), data.get('nation'), data.get('currency'), category_id, data.get('user_id'), data.get('container_id'), data.get('color'), data.get('type_ui')]
    );
    }
